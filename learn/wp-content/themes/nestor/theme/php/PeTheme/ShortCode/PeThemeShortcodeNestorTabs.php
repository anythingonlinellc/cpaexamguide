...
class PeThemeShortcodeNestorTabs extends PeThemeShortcode {
	...
	public function container($atts,$content=null,$code="") {
		...
		if (peTheme()->template->exists("shortcode","tab")) {
			...
			while ($tab = array_shift($this->items)) {
				$id = "{$commonID}_{$count}";
				$head .= sprintf('<li%s><a href="#tab-%s" data-toggle="tab">%s</a></li>',$count == 1 ? ' class="active"' : '',$id,$tab->title);
				$body .= sprintf('<div class="tab-pane fade in %s" id="tab-%s">%s</div>',$count == 1 ? ' active' : '',$id,$tab->body);
				$count++;
			}
