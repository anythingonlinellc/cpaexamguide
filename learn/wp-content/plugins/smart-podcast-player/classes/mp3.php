...
class SPP_MP3 {
	...
	/**
	 * Get type of url that an input is - feed or mp3
	 * 
	 * @param  string $url
	 * @return string      feed | rss
	 */
	...
	/**
	 * Get the first URL of a track
	 * @param  string $feed_url URL of the RSS Feed
	 * @return URL           The URL of a track
	 */
	public static function get_feed_first_track_url( $feed_url ) {
		...
		if( strpos( $feed_url, 'http://soundcloud.com/' ) !== false || strpos( $feed_url, 'http://soundcloud.com/sets/' ) !== false || strpos( $feed_url, 'https://soundcloud.com/' ) !== false || strpos( $feed_url, 'https://soundcloud.com/sets/' ) !== false ) {
			$tracks = SPP_Core::get_soundcloud_tracks( $feed_url );
			$audio_url = $tracks[0]->uri;
		} else {
			$tracks = SPP_Core::get_rss_tracks( $feed_url );
