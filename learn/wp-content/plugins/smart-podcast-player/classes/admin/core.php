...
class SPP_Admin_Core {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_nopriv_get_colors', array( $this, 'ajax_get_colors' ) );
		add_action( 'wp_ajax_get_colors', array( $this, 'ajax_get_colors' ) );
	...
	public function enqueue_admin_scripts() {
		...
		wp_enqueue_script( $this->plugin_slug . '-admin-script', SPP_ASSETS_URL . 'js/admin-spp.js', array( 'jquery', 'wp-color-picker' ), SPP_Core::VERSION );

		wp_localize_script( $this->plugin_slug . '-admin-script', 'Smart_Podcast_Player_Admin', array(
	...
	public function ajax_get_colors() {

		require_once( SPP_PLUGIN_BASE . 'classes/player-css.php' );		
		require_once( SPP_PLUGIN_BASE . 'classes/player-link-css.php' );		
		require_once( SPP_PLUGIN_BASE . 'classes/track-css.php' );

		$output = '';
		if( isset( $_POST['spp_colors'] ) ) {
			foreach( $_POST['spp_colors'] as $color ) {
				$c = new SPP_Player_CSS( $color );
				$output .= $c->render();
			}
		}

		if( isset( $_POST['spp_link_colors'] ) ) {
			foreach( $_POST['spp_link_colors'] as $color ) {
				$c = new SPP_Player_Link_CSS( $color );
				$output .= $c->render();
			}
		}

		if( isset( $_POST['stp_colors'] ) ) {
			foreach( $_POST['stp_colors'] as $color ) {
				$c = new SPP_Track_CSS( $color );
				$output .= $c->render();
			}
		}
